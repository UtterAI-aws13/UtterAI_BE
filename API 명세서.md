...
| 상태 | 기능 | 사용자 | Method | URL | Query Param | Request Body | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 미구현 | 분석 결과 조회 | 치료사, 관리자 | GET | `/api/v1/analysis-results/{resultId}` | - | - | 전체 분석 결과 조회 |
| 미구현 | 세션별 분석 결과 조회 | 치료사, 관리자 | GET | `/api/v1/sessions/{sessionId}/analysis-results` | - | - | 세션별 분석 결과 조회 |
...
| 미구현 | 언어 지표 조회 | 치료사, 관리자 | GET | `/api/v1/analysis-results/{resultId}/metrics` | - | - | 발화량, MLU, TTR 등 조회 |
...
| 상태 | 기능 | 사용자 | Method | URL | Query Param | Request Body | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 미구현 | SOAP Note 초안 생성 | 치료사 | POST | `/api/v1/soap-notes/generate` | - | `sessionId`, `transcriptId`, `clinicalAnalysisJobId` | 분석 결과 기반 초안 생성 |
| 미구현 | SOAP Note 목록 조회 | 치료사, 관리자 | GET | `/api/v1/soap-notes` | `sessionId`, `childId`, `status`, `page`, `size` | - | SOAP Note 목록 조회 |
| 미구현 | SOAP Note 상세 조회 | 치료사, 관리자 | GET | `/api/v1/soap-notes/{noteId}` | - | - | SOAP Note 상세 조회 |
...
| 상태 | 기능 | 사용자 | Method | URL | Query Param | Request Body | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 미구현 | 리포트 생성 | 치료사 | POST | `/api/v1/reports` | - | `sessionId`, `resultId`, `templateType` | 분석 결과 기반 리포트 생성 |
| 미구현 | 리포트 목록 조회 | 치료사, 관리자 | GET | `/api/v1/reports` | `childId`, `page`, `size` | - | 리포트 목록 조회 |
| 미구현 | 리포트 상세 조회 | 치료사, 보호자, 관리자 | GET | `/api/v1/reports/{reportId}` | - | - | 리포트 상세 조회 |
| 미구현 | 리포트 수정 | 치료사 | PATCH | `/api/v1/reports/{reportId}` | - | `title`, `content`, `memo` | 리포트 수정 |
| 미구현 | 리포트 PDF 다운로드 | 치료사, 보호자 | GET | `/api/v1/reports/{reportId}/download` | - | - | PDF 다운로드 |
